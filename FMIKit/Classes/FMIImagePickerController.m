...
- (UIActionSheet *)actionSheetWithCameraButtonForImage:(UIImage *)image {
    return [[UIActionSheet alloc] initWithTitle:nil
                         ...
                         destructiveButtonTitle:[self destructiveButtonTitleForImage:image]
                              otherButtonTitles:NSLocalizedStringFromTableInBundle(@"Choose Photo", @"FMIKitLocalizable", [FMIKitPrivates resourcesBundle], @"Used for button title: Choose an existing photo"), NSLocalizedStringFromTableInBundle(@"Take Photo", @"FMIKitLocalizable", [FMIKitPrivates resourcesBundle], @"Used for button title: Take a new photo"), nil];
}
...
- (void)updateActionSheetButtonIndexesForImage:(UIImage *)image {
    ...
    self.imageButtonIndexes = imageButtonIndexes;
}
...
- (NSString *)destructiveButtonTitleForImage:(UIImage *)image {
    return ((!image) ? nil : NSLocalizedStringFromTableInBundle(@"Delete Photo", @"FMIKitLocalizable", [FMIKitPrivates resourcesBundle], @"Used for button title: Delete current photo"));
}
